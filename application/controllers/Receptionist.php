...
public function add_patient() {
    // Fetch the list of doctors and specialists
    ...
    if ($this->input->server('REQUEST_METHOD') == 'POST') {
        // Fetching patient data from POST data
        $patientData = array(
        ...
        if (!empty($patientData['first_name']) && !empty($patientData['dob']) && 
            !empty($patientData['age']) && !empty($patientData['phone']) && 
            !empty($patientData['email']) && !empty($patientData['doctor_id'])) {
    ...
    public function getDoctors() {
        ...
        $doctors = $this->Receptionist_model->fetchDoctorsBySpecialist($specialistId);
...
if ($patient_id || $patient_name) {
    ...
    if ($patient_id) {
        ...
        $patient['details'] = $this->Receptionist_model->Patient($patient_id);
        // Check if the patient exists
        if (empty($patient['details'])) {
            $this->session->set_flashdata('message', 'Patient not found.');
        }
        
    } elseif ($patient_name) {
        // Otherwise, treat it as patient name
        $patient['details'] = $this->Receptionist_model->Patient(null, $patient_name);
        // Check if the patient exists
        if (empty($patient['details'])) {
            $this->session->set_flashdata('message', 'Patient not found.');
        }
...
public function add_doctor() {
    ...
    if ($this->input->server('REQUEST_METHOD') == 'POST') {
        ...
        $existingUser = $this->Receptionist_model->get_user_by_email($email);
        ...
        if ($user_id) {
            ...
            switch ($userData['role']) {
                case 'doctor':
                    if ($this->Receptionist_model->add_new_doctor($doctorData)) {
                        $this->session->set_flashdata('success', 'Doctor added successfully!');
                    } else {
                        $this->session->set_flashdata('error', 'Failed to add doctor. Please try again.');
                    }
                    break;
                ...
                case 'inventory_manager':
                    ...
                    if ($this->Receptionist_model->add_new_inventory_manager($inventoryData)) {
                        $this->session->set_flashdata('success', 'Inventory Manager added successfully!');
                    } else {
                        $this->session->set_flashdata('error', 'Failed to add inventory manager. Please try again.');
                    }
                    break;
                ...
                case 'receptionist':
                    ...
                    if ($this->Receptionist_model->add_new_receptionist($receptionistData)) {
                        $this->session->set_flashdata('success', 'Receptionist added successfully!');
                    } else {
                        $this->session->set_flashdata('error', 'Failed to add receptionist. Please try again.');
                    }
                    break;

                default:
                    $this->session->set_flashdata('error', 'Invalid role selected. Please try again.');
                    break;
            }
